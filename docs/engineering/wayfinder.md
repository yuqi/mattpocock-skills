...
| A cleared wayfinder map | [to-spec](https://aihero.dev/skills-to-spec), then [to-tickets](https://aihero.dev/skills-to-tickets) and [ticket-implement](https://aihero.dev/skills-ticket-implement) |
...
The map and its tickets live on the repo's local Markdown Issue Tracker, so wayfinder needs the wiring that [setup-matt-pocock-skills](https://aihero.dev/skills-setup-matt-pocock-skills) lays down. Its `Wayfinding operations` section defines the map, child tickets, blocking edges, frontier, claim, and resolve actions. Wayfinder resolves that doc through the pointer in your `CLAUDE.md` or `AGENTS.md`; a missing operation is a setup error, not a fallback branch.

The tracker is not decoration. Local ticket files carry their blocking edges and state, and the configured Frontier operation scans them to determine what is takeable.

## The map, the fog, and the frontier

The **map** is one `map.md` file; its decision tickets are separate files under its sibling `issues/` directory. It is an **index, not a store**: a decision lives in exactly one place, its ticket, and the map only gists it and links. A session loads the map at low resolution and zooms into individual tickets on demand, which is what lets a map keep growing without every session paying for its whole history.
...
- **Decisions so far**: one line per resolved ticket, each linking to where the detail actually lives.
...
- **Out of scope**: work ruled beyond the destination. Fog only ever gathers *toward* the destination, so out-of-scope work is resolved off the route and never graduates.

The **frontier** is the open, unblocked, unclaimed tickets (the edge of the known). Blocking lives in each ticket's `Blocked by:` line, and a session claims a ticket by updating its `Status:` before doing any work. Tickets are referred to by name throughout, never by a bare number or path.

## The four decision-ticket types

Every ticket records one of the four types in its `Type:` field, and is either **[HITL](https://www.aihero.dev/ai-coding-dictionary/human-in-the-loop)** (worked with a human who speaks for themselves) or **[AFK](https://www.aihero.dev/ai-coding-dictionary/afk)**, driven by the agent alone. A HITL ticket only resolves through the live exchange; an agent that answers its own [grilling](https://www.aihero.dev/ai-coding-dictionary/grilling) questions has broken it.
...
| `research` | AFK | A fact outside the working directory is blocking a decision. | One [research](https://aihero.dev/skills-research) [subagent](https://www.aihero.dev/ai-coding-dictionary/subagent) returns findings to the owning wayfinder session, which records the resolution |
...
Every type follows one ticket per session.
...
No. Wayfinder's tickets are decision tickets, and by the time the map resolves they are all resolved too. What is left is a map full of linked decisions, which is not a build plan. [to-spec](https://aihero.dev/skills-to-spec) collapses those linked decisions into one spec (`/to-spec .scratch/<effort>/map.md`) and [to-tickets](https://aihero.dev/skills-to-tickets) slices that into tracer-bullet implementation tickets. Looping the map straight into [ticket-implement](https://aihero.dev/skills-ticket-implement) skips the collapse and throws the linked detail away. Go straight to [implement](https://aihero.dev/skills-implement) only when the effort turned out genuinely small. People do run the abbreviated pipeline and report it working; the two extra steps buy you an explicit spec artifact that a reviewer or a colleague can read, which matters more the less solo you are.
...
No. Local wayfinder is a single-writer workflow: one session claims and resolves one decision ticket, then the next session reads the updated map. A research subagent may gather facts for that one ticket, but it returns them to the owning session instead of writing tracker files. Parallel implementation remains a separate downstream branch and worktree workflow after the map has become a spec and implementation tickets.

**Where are the map and tickets stored?**
The local Issue Tracker writes `.scratch/<effort>/map.md` and one decision ticket per file under `.scratch/<effort>/issues/`. The map links to resolved detail instead of copying it.
...
There is no official guidance, and the agent's instinct is unhelpful: it tends to design around the bad decision rather than challenge it, so you have to steer manually. What does work is telling wayfinder plainly what changed; it updates the map, revises the affected tickets, and updates already-resolved answers. Scope changes mid-map are recoverable. A map you *designed* to change is a scoping smell.
...
- Scanning ticket `Status:` and `Blocked by:` fields identifies the frontier without loading every ticket body.
- A session resolves one ticket, records its answer and resolved state, and leaves one line on the map's *Decisions so far*. Then it stops.
