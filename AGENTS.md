...
`scripts/link-skills.sh` is an explicit, user-requested maintenance action for linking every skill into the local harness directories (`~/.agents/skills` for Codex and compatible harnesses, plus `~/.claude/skills` for Claude Code). Run it only when the current user specifically asks to relink skills; adding, removing, or renaming a skill does not authorize it. Before running, list every existing target it would replace. If any target is not a symlink, stop and ask for direction instead of deleting it. Each linked entry is a symlink into this repo, so a `git pull` keeps it current.
