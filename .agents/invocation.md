...
- **User-invoked**: reachable **only by the human typing its name**. Set `policy.allow_implicit_invocation: false` in `agents/openai.yaml` for Codex, then mirror it with `disable-model-invocation: true` in the frontmatter for Claude Code compatibility. The `description` is **human-facing**: a one-line summary read by a person browsing slash-commands. Strip trigger lists ("Use when the user says…").
- **Model-invoked**: reachable by **model or user**. The default: omit the `policy` block from `agents/openai.yaml`, then omit `disable-model-invocation` from the frontmatter as its Claude Code mirror. The `description` is **model-facing** and keeps rich trigger phrasing ("Use when the user wants…, mentions…, asks for…") so auto-invocation fires. The test for whether a skill should stay model-invoked: _could the model usefully reach for this autonomously?_ (Reuse is the reason to extract a skill, not the test.)
...
Every skill carries an `agents/openai.yaml` beside its `SKILL.md`. It is the authoritative invocation metadata and holds Codex UI fields: `interface.display_name` and `interface.short_description` for the skill picker, and, for user-invoked skills, `policy.allow_implicit_invocation: false`. Keep the Claude Code frontmatter mirror in sync: a skill is user-invoked in both harnesses or neither.
