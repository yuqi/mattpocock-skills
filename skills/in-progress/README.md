...
- **[spec-implement](./spec-implement/SKILL.md)**: Implement an explicitly referenced local spec through parallel worktree workers, serialized ticket acceptance, cumulative repair, and final full-suite validation, with per-spec saved execution preferences and task-aware model selection. Model-invoked; requires `ticket-review`, `spec-review`, `code-review`, and `tdd`.
