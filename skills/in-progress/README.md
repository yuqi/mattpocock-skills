...
- **[spec-implement](./spec-implement/SKILL.md)**: Implement an explicitly referenced local spec and its prepared tickets through a sequential queue, persisted ticket acceptance, cumulative review and repair, and final full-suite validation. Model-invoked; requires `ticket-review`, `spec-review`, `code-review`, and `tdd`.
