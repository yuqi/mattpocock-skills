...
- **[spec-implement](./spec-implement/SKILL.md)**: Implement an explicitly referenced local spec through parallel worktree workers, serialized integration with persisted ticket acceptance, cumulative review and repair, and final full-suite validation on the current branch. Model-invoked; requires `ticket-review`, `spec-review`, `code-review`, and `tdd`.
