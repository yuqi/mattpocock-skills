...
- Every Markdown heading shown as inline code in this contract is a machine-readable identifier. Preserve its exact English text and heading level in every Issue Tracker artifact.
- Specs and implementation tickets keep their authoritative acceptance set or checklist under `## Acceptance Criteria`.
...
- **Ticket-scoped commit**: every implementation, repair, and ticket Review-result commit adds `Issue-Tracker-Ticket: .scratch/<feature-slug>/issues/<NN>-<slug>.md`. When the sibling spec exists, it also adds `Issue-Tracker-Spec: .scratch/<feature-slug>/spec.md`. Keep the applicable lines together in one contiguous trailer block.
- **Review-result commit**: a ticket Review-result commit adds `Issue-Tracker-Review-Result: ticket`; a spec Review-result commit adds `Issue-Tracker-Review-Result: spec` together with its `Issue-Tracker-Spec:` reference. A migration that carries forward an unchanged current ticket Review also adds the ticket marker. Implementation commits and metadata edits that do not persist a Review result do not. Commit only the reviewed ticket or spec file and preserve unrelated index and worktree changes.
...
- **Commit hashes**: use commit identities only as internal review boundaries. Do not persist them in Issue Tracker files or include them in workflow reports.
...
- **Map**: `.scratch/<effort>/map.md`, with `## Destination`, `## Notes`, `## Decisions so far`, `## Not yet specified`, and `## Out of scope` sections.
- **Child ticket**: `.scratch/<effort>/issues/NN-<slug>.md`, numbered from `01`, with the question under `## Question`. A `Type:` line records the ticket type (`research`/`prototype`/`grilling`/`task`); a `Status:` line records `claimed`/`resolved`.
